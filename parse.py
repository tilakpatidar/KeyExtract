...
def getFrequency():
	...
	try:
		list1=removeVerbs(filter_data(__soup.find_element_by_tag_name("body").text))
...
def getOccuredWords(data):
	...
	try:
		...
		return list(set(__soup.find_element_by_tag_name("body").text.split(" ")).intersection(set(data.split(" "))))
	except Exception as e:
